    ...
    window.Prevue = class {
        ...
        init() {
            ...
            this.retrieveOptions(options => {
                ...
                // OTTIMIZZAZIONE: Disattivazione per siti pesanti
                const heavySites = [
                    'webench.ti.com',
                    'twitter.com',
                    'youtube.com',
                    ...
                    'tiktok.com',
                    'lcsc.com'
                    // aggiungi altri siti problematici qui
                ];

                // Disattiva le animazioni sui siti pesanti
                const isHeavySite = heavySites.some(site => window.location.href.includes(site));
                if (isHeavySite) {
                    console.log('Prevue: Site detected as heavy, optimizing performance');
                    this.el.sidePreview.style.transition = 'none';

                    // Riduce la frequenza di eventi su siti pesanti
                    const originalListen = this.listen;
                    this.listen = (els, event, handler) => {
                        if (['mousemove', 'mouseover'].includes(event)) {
                            // Wrapper con throttling per eventi ad alta frequenza
                            const throttledHandler = (e) => {
                                const now = Date.now();
                                if (!this._lastEventTime) this._lastEventTime = {};
                                if (!this._lastEventTime[event]) this._lastEventTime[event] = 0;

                                if (now - this._lastEventTime[event] < 150) return;
                                this._lastEventTime[event] = now;

                                handler(e);
                            };

                            originalListen.call(this, els, event, throttledHandler);
                        } else {
                            // Per altri eventi usa la funzione originale
                            originalListen.call(this, els, event, handler);
                        }
                    };
                }
            })
            ...
            // }
            let popup = document.querySelector('#prevue--wrapper');
            let cs = document.querySelector('contextsearch-widgets');
            ...
            const handleMouseOut = () => {
                addStyle(window.location.href.includes("google.com/search") ? '#prevue--wrapper { width: 610px !important; }' : '#prevue--wrapper { width: 50% !important; }');
            };
            ...
            function addEventsToElement() {
                ...
                cs.addEventListener('mouseover', handleMouseOver);
                cs.addEventListener('mouseout', handleMouseOut);
            }

            // OTTIMIZZAZIONE: MutationObserver migliorato
            function observeElementCreation() {
                // Limitiamo l'esecuzione dell'observer a max 5 secondi
                const maxObserveTime = 5000;
                const startTime = Date.now();

                // Verifica immediata
                popup = document.querySelector('#prevue--wrapper');
                cs = document.querySelector('contextsearch-widgets');
                ...
                const observer = new MutationObserver(function (mutations) {
                    // Esci se abbiamo superato il tempo massimo
                    if (Date.now() - startTime > maxObserveTime) {
                        observer.disconnect();
                        return;
                    }

                    // Controlla solo alcune mutazioni per performance
                    for (let i = 0; i < Math.min(3, mutations.length); i++) {
                ...
                // Usa un timeout per limitare la durata massima dell'observer
            ...
            if (popup || cs) {
                addEventsToElement(popup);
            } else {
                observeElementCreation();
                // setTimeout(observeElementCreation, 2000);
            }
        }
        ...
        // OTTIMIZZAZIONE: imageZoomPanningHandler migliorato con throttling
        imageZoomPanningHandler(e) {
            // Aggiungi throttling per ridurre la frequenza degli aggiornamenti
            const now = Date.now();
            if (now - this._lastPanningTime < 60) return; // Throttling a 16fps invece di 60fps
