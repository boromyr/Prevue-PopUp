let options={target:"both",targetLinkTypes:"both",openPosition:"right",openAnimation:!0,displayUrl:!0,urlPosition:"top",escCloseTrigger:!0,outsideClickCloseTrigger:!0,outsideScrollCloseTrigger:!1,width:50,widthUnit:"%",triggerOpenDelay:50,triggerReleaseDelay:400,triggers:[{key:"",action:"drag"},{key:"ctrl",action:"mouseover"}]};chrome.storage.sync.get(null).then(e=>{options={...options,...e},chrome.storage.sync.set(options),document.addEventListener("change",e=>{"SELECT"===e.target.tagName&&((e=e.target).name.startsWith("triggers[")?options.triggers[e.dataset.index][e.dataset.prop]=e.value:options.triggers[e.name]=e.value,chrome.storage.sync.set(options))}),document.addEventListener("click",e=>{"INPUT"===e.target.tagName&&(options[e.target.name]="checkbox"===e.target.type?e.target.checked:e.target.value,chrome.storage.sync.set(options))}),updateInputValues()});const currentSettings={},title=(document.querySelectorAll("[name]").forEach(o=>{"META"!==o.tagName&&chrome.storage.sync.get([o.name],e=>{var t="checkbox"===o.type?"checked":"value";void 0===e[o.name]?(chrome.storage.sync.set({[o.name]:defs[o.name]}),o[t]=defs[o.name],currentSettings[o.name]=defs[o.name]):(o[t]=e[o.name],currentSettings[o.name]=e[o.name])})}),setTimeout(()=>{document.querySelectorAll("[name]").forEach(t=>{var e;"META"!==t.tagName&&(e="checkbox"===t.type?"click":"change",t.addEventListener(e,e=>{e="checkbox"===t.type?e.target.checked:e.target.value,chrome.storage.sync.set({[t.name]:e},()=>{chrome.runtime.sendMessage({action:"reinjectPrevueEverywhere"})})}))}),currentSettings.ua=navigator.userAgent,document.querySelector("#contact-link").href+="?prevue="+btoa(JSON.stringify(currentSettings))},1e3),document.querySelector("h1"));function updateInputValues(){Object.keys(options).map(e=>{var t=document.querySelector(`[name="${e}"]`);t&&("checkbox"===t?.type?t.checked=options[e]:t.value=options[e])}),updateTriggersSection()}function updateTriggersSection(){var e,t=document.querySelector("#triggers");let o="",r=0;for(e of options.triggers)o+=`
