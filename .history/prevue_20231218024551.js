	...
	((window.Prevue = class {
		...
		init() {
			this.listenForBackgroundMessages(),
				...
				/^#prevue:sorry/.test(location.hash || "") &&
				(alert(
					...
					switch (i.action) {
						...
						case "mouseover":
							this.listenTo("mouseover", (e) => {
								(!e[i.key + "Key"] && i.key) || this.searchLinkAndTriggerPopup(e, !1, !0);
							});
							break;
						case "drag":
						...
						document.addEventListener("mouseover", (i) => {
							document.addEventListener("keydown", (e) => {
								"ArrowUp" !== e.key ||
									...
									e.shiftKey ||
									e.metaKey ||
									[".jpg", ".png", ".gif", ".jpeg", ".bmp"].some((e) => i.target.href.endsWith(e)) ||
									(this.searchLinkAndTriggerPopup(i, !0, !0), this.url && e.preventDefault());
							});
						});
					}),
					window.location.href.includes("google.com/search?q") && !window.location.href.includes("tbm="))
			) {
				let i;
				this.listenTo("mouseover", (e) => {
					e.target.href;
		...
		getImageZoomPerc() {
			var e = this.el.sidePreviewImage,
				i = (e.clientWidth / e.naturalWidth) * 100,
				e = (e.clientHeight / e.naturalHeight) * 100;
			return Math.round(Math.min(i, e));
		}
		retrieveOptions(i) {
			try {
		...
		imageZoomPanningHandler(i) {
			var t = this.el.sidePreviewImage;
			if (t.dataset?.panning) {
				var s = this.el.sidePreviewImageWrapper.offsetWidth,
					r = this.el.sidePreviewImageWrapper.offsetHeight,
					n = t.naturalWidth > s,
					e = t.naturalHeight > r;
				if (
					(n &&
					...
					let e = i.x - this.panningXOffset;
					n = (e = (e = e < 15 ? 0 : e) > s - 15 ? s : e) / s;
					t.style.left = -n * (t.offsetWidth - s) + "px";
				} else t.style.left = s / 2 - t.offsetWidth / 2 + "px";
				if (e) {
					let e = i.y - this.panningYOffset;
					n = (e = (e = e < 15 ? 0 : e) > r - 15 ? r : e) / r;
					t.style.top = -n * (t.offsetHeight - r) + "px";
				} else t.style.top = r / 2 - t.offsetHeight / 2 + "px";
			}
		}
		bg(e, i = function () { }) {
