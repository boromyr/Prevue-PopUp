	...
	window.Prevue = class {
		...
		init() {
			...
			this.retrieveOptions((options) => {
				...
				this.targetLinks = ["both", "internal", "external"].includes(this.options.targetLinkTypes);
				this.targetImages = ["both", "links", "images"].includes(this.options.target);
				// this.targetImages = false;
			...
			// 	alert(
			// 		"Mi dispiace, il link precedente non funziona con Prevue. Ha cercato di reindirizzarti all'URL aperto. Sei stato reindirizzato alla pagina precedente per tua comodità. Purtroppo, non c'è modo di impedire che ciò accada."
			// 	);€
		...
		setupTriggers() {
					...
					document.addEventListener("mouseover", (i) => {
						document.addEventListener("keydown", (e) => {
							if ("ArrowUp" === e.key && e.altKey && !e.ctrlKey && !e.shiftKey && !e.metaKey) {
								if (this.url && !this.url.endsWith(".jpg") && !this.url.endsWith(".png") && !this.url.endsWith(".gif")) {
									this.searchLinkAndTriggerPopup(i, !0, !0), this.url && e.preventDefault();
									// alert(this.url)
			...
			if (window.location.href.includes("google.com/search?q") && !window.location.href.includes("tbm=")) {
				...
				this.listenTo("mouseover", (e) => {
					...
					const mouseX = e.clientX; // Posizione orizzontale del mouse rispetto alla finestra del browser
					const windowWidth = window.innerWidth; // Larghezza della finestra del browser
		...
		switchSides() {
			...
			// Reimpostazione di parte della cache.
		...
		searchLinkAndTriggerPopup(e, isDragging = false, recordEvent = false) {
			...
			if (url && this.url !== url) {
				if (
					this.options.targetLinkTypes === "both" ||
		...
		prebuildHtml(defaultWidth) {
			...
			// Crea il wrapper dei pulsanti di azione.
		...
		sidePreview(type) {
			// Se il mouse è posizionato sul lato destro della pagina, aggiungi una classe CSS.
		...
		openIframePopup() {
			...
			this.bg("disableCsp", () => {
				...
				this.enableCspTimeout = setTimeout(() => this.bg("enableCsp"), 10e3);
			});
		}
		...
		shouldOpenOnTheRight() {
			...
			return this.options.openPosition === "right" || this.event.clientX <= window.innerWidth / 2;
		}
		...
		isExternal() {
			...
			// Inizia con qualcosa di diverso da http o //
			if (!this.url.toLowerCase().startsWith("http") && /^\/?[^/]+/.test(this.url)) {
				return false;
			}

			// Controlla se contiene la posizione originale.
		...
		imageZoomPanningHandler(e) {
			...
			if (shouldHandleYPanning && this.panningYOffset === undefined) {
				this.panningYOffset =
					this.options.urlPosition === "top" && this.options.displayUrl ? this.el.sidePreviewTitleWrapper.offsetHeight : 0;
			}
		...
		/**
		 * Semplice strumento di debug per misurare i tempi.
		 */
		ts(append = "") {
			if (this.prevTs) {
		...
		/**
		 * Le chiamate API chrome.*falliscono quando l'estensione viene aggiornata o ricaricata,
		 * che sostanzialmente si traduce nel fatto che questo specifico
		 * l'iniezione di script di contenuto non è più utilizzabile.
		 * Quindi sto invalidando i suoi ascoltatori di eventi.
		 *
		 * Tieni presente che in genere avrà luogo una successiva immissione di contenuto
		 * quando ciò accade, che sostanzialmente "sostituisce" questo.
		 * Questo controllo è stato aggiunto perché lo sono effettivamente
