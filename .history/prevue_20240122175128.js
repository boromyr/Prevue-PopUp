(() => {
    ...
    window.Prevue = class {
        ...
        init() {
            // console.log(`💥 Prevue Popup ${reloaded ? 're' : ''}loaded successfully.`)s
                    ...
                    document.addEventListener("mouseover", (i) => {
                        document.addEventListener("keydown", (e) => {
                            if (
                                ...
                                this.listener(i, !0, !0); //, this.url && e.preventDefault();
            ...
            if (
                ...
                !window.location.href.includes("tbm=")
            ) {
                let i;
                this.listenTo("mouseover", (e) => {
            ...
            this.retrieveOptions((options) => {
                ...
                if (this.options.dragTrigger) {
                    ...
                    this.listenTo("dragend", (e) => {
                        if (
                            this.dragDelta >= this.options.dragTriggerOpenDelay &&
        ...
        listener(e, isDragging = false, recordEvent = false) {
            ...
            if (this.targetImages && e.target.tagName === "IMG") {
                ...
                const a =
                    e.target.tagName === "A" ? e.target : e.target.closest("a[href]");
                url = a?.href;
                type = "url";
            }
        ...
        prebuildHtml(defaultWidth) {
            ...
            document
                .querySelectorAll("[id=prevue--wrapper]")
            ...
            if (this.options.openAnimation) {
                this.el.sidePreview.style.transition =
                    "opacity .2s, left .2s, right .2s";
            }
            ...
            this.el.sidePreviewImage.onclick = (e) => {
                ...
                if (this.getImageZoomPerc() < 100 || image.dataset.panning) {
                    image.dataset.panning =
                        image.dataset.panning === "true" ? "false" : "true";
                }
                setTimeout(() => this.imageZoomPanningHandler(e), 1);
                setTimeout(() => this.setTitle(` (${this.getImageZoomPerc()}%)`), 100);
            };
            this.el.sidePreviewImage.onload = () =>
                this.setTitle(` (${this.getImageZoomPerc()}%)`);
        ...
        sidePreview(type) {
            ...
            this.el.sidePreview.classList.toggle(
                "prevue--right",
                this.shouldOpenOnTheRight()
            );
            if (type === "image") {
                this.setTitle();
        ...
        openIframePopup() {
            ...
            this.el.sidePreview.classList.add("prevue--visible");
            this.bg("disableCsp", () => {
                this.el.sidePreviewIframe.src = `${this.iframeBaseUrl}?${btoa(
                    this.url
                )}`;
        ...
        shouldOpenOnTheRight() {
            ...
            return (
                this.options.openPosition === "right" ||
        ...
        setTitle(append = "") {
            this.el.sidePreviewTitleWrapper.children[0].innerHTML =
                this.visualUrl(append);
        ...
        isExternal() {
            ...
            return !new RegExp(`^(http)?s?:?//${location.hostname}`, "i").test(
                this.url
            );
        }
        isInternal() {
        ...
        imageZoomPanningHandler(e) {
            ...
            if (shouldHandleXPanning && this.panningXOffset === undefined) {
                this.panningXOffset = this.el.sidePreview.classList.contains(
                        "prevue--right"
                    ) ?
            ...
            if (shouldHandleYPanning && this.panningYOffset === undefined) {
                this.panningYOffset =
                    this.options.urlPosition === "top" && this.options.displayUrl ?
