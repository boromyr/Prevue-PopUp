"about:blank" !== location.href &&
	((window.Prevue = class {
		...
		setupTriggers() {
			...
			// 					this.dragDelta >= this.options.triggerOpenDelay &&r
					...
					document.addEventListener("mouseover", (i) => {
						document.addEventListener("keydown", (e) => {
							if ("ArrowUp" === e.key && e.altKey && !e.ctrlKey && !e.shiftKey && !e.metaKey) {
								const imageFormats = [".jpg", ".png", ".gif", ".jpeg", ".bmp"];
								if (!imageFormats.some((format) => i.target.href.endsWith(format))) {
			...
			if (window.location.href.includes("google.com/search?q") && !window.location.href.includes("tbm=")) {
				document.addEventListener("keydown", (e) => {
					if (e.altKey && e.key === "ArrowUp" && !e.ctrlKey && !e.shiftKey && !e.metaKey) {
						e.preventDefault();
		...
		restyleEmbeddedSitesScrollbars() {
            ...
            //   body::-webkit-scrollbar { background: transparent !important; width: 11px !important; height: 6px !important; }
